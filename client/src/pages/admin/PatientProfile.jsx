...
const AdminPatientProfile = () => {
  ...
  const fetchPatientData = async () => {
    setLoading(true);
    try {
      const { data } = await adminService.getPatientById(patientId);
  ...
  if (!patient) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center animate-fade-in">
        ...
        <Button onClick={() => navigate('/admin/patients')} variant="primary" className="shadow-lg shadow-primary/20">
          <ChevronLeft className="w-4 h-4 mr-2" /> Return to Patients Management
        </Button>
      </div>
    );
  }
  ...
  return (
    <PageTransition className="max-w-6xl mx-auto space-y-8 pb-10">
      ...
      >
        <ChevronLeft className="w-4 h-4 mr-1" /> Back to Patients Management
      </button>
        ...
        <div className="flex flex-col md:flex-row gap-8 items-center md:items-start relative z-10">
          ...
          <div className="flex-1 text-center md:text-left space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 pt-4 border-t border-slate-100 w-full">
              <Link to={`/admin/chat/dummy_${patient._id}`}>
                <Button variant="outline" className="border-slate-200 hover:bg-slate-50">
                  <Mail className="w-4 h-4 mr-2 text-slate-500" /> Message
                </Button>
              </Link>
              <Button variant={patient.isActive ? "danger" : "success"} className="shadow-sm">
                <ShieldCheck className="w-4 h-4 mr-2" />
                {patient.isActive ? 'Suspend Account' : 'Activate Account'}
              </Button>
            </div>
          </div>
        </div>
                ...
                appointments.map(appt => (
                  <div key={appt._id} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                    ...
                    <div className="flex flex-col gap-2 w-full md:w-auto">
                      <Badge variant="outline">Viewed by Admin</Badge>
                    </div>
                  </div>
                ))
              ) : (
                  ...
                  {prescriptions.map(rx => (
                    <div key={rx._id} className="bg-white rounded-2xl p-6 border border-slate-100 shadow-sm flex flex-col h-full">
                      ...
                      <div className="mt-auto pt-4 border-t border-slate-100">
                        <Link to={`/admin/prescriptions/${rx._id}`}>
                          <Button variant="outline" className="w-full text-sm disabled:opacity-50">View details disabled</Button>
                        </Link>
                      </div>
                    </div>
                  ))}
